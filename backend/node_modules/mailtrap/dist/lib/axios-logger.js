 ...
 */
function formatFieldError(field, fieldErrors) {
...
function extractFieldErrors(errorsObj) {
    return Object.entries(errorsObj)
        ...
        .map(([field, fieldErrors]) => formatFieldError(field, fieldErrors))
        .filter((msg) => msg.length > 0);
}
/**
 * Gets identifier (email or id) from an error object.
 */
function getErrorIdentifier(errorObj) {
    return (errorObj.email || errorObj.id || "");
}
/**
 * Extracts errors from nested "errors" property.
 */
function extractNestedErrors(errorObj) {
    if (errorObj.errors &&
        typeof errorObj.errors === "object" &&
        !Array.isArray(errorObj.errors)) {
        return extractFieldErrors(errorObj.errors);
    }
    return null;
}
/**
 ...
 */
function extractDirectErrors(errorObj) {
    const directErrors = Object.entries(errorObj)
        .filter(([field]) => field !== "email" && field !== "id" && field !== "errors")
...
function formatErrorMessage(errorObj) {
    ...
    const itemMessages = extractNestedErrors(errorObj) || extractDirectErrors(errorObj);
 ...
 */
function extractMessagesFromErrorObjects(errorObjects) {
 ...
 */
function extractErrorMessage(data, defaultMessage) {
    ...
    if ("errors" in data && data.errors) {
        ...
        if (Array.isArray(errors) && errors.length > 0) {
            ...
            if (typeof errors[0] === "object" && errors[0] !== null) {
                const extracted = extractMessagesFromErrorObjects(errors);
                if (extracted) {
                    return extracted;
                }
            }
        ...
        if (typeof errors === "object" &&
            ...
            const errorNames = hasErrorProperty(errors, "name") && errors.name.join(", ");
            const errorBase = hasErrorProperty(errors, "base") && errors.base.join(", ");
            if (errorNames)
                return errorNames;
            if (errorBase)
                return errorBase;
            // extract field-specific errors (e.g., { "email": ["is invalid", ...] })
            const fieldMessages = extractFieldErrors(errors);
